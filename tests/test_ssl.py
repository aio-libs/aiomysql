    ...
    async with create_pool(**mysql_server['conn_params'], loop=loop) as pool:
        async with pool.get() as conn:
            async with conn.cursor() as cur:
                ...
                # Spot check the answers, we should at least have mysql and information_schema
                assert 'mysql' in values, 'Could not find the "mysql" table'
                assert 'information_schema' in values, 'Could not find the "mysql" table'
                ...
                # The context has TLS
                assert value[1].startswith('TLS'), 'Not connected to the database with TLS'
